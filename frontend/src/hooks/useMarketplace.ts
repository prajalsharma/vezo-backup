...
export function useActiveListings() {
  ...
  // Step 1: batch-fetch active listings for both collections in one multicall
  const { data: batchResults, isLoading: batchLoading, refetch } = useReadContracts({
    contracts: [
      {
        address: marketplaceAddress,
        abi: MARKETPLACE_ABI,
        functionName: "getActiveListings",
        args: [veBTCAddress, 0n, ACTIVE_LISTINGS_LIMIT],
        chainId,
      },
      {
        address: marketplaceAddress,
        abi: MARKETPLACE_ABI,
        functionName: "getActiveListings",
        args: [veMEZOAddress, 0n, ACTIVE_LISTINGS_LIMIT],
        chainId,
      },
    ] as const,
  ...
  // Flatten the two results into a single array, tagging each with listingId.
  // getActiveListings returns tuples in storage order — we cross-reference
  // by iterating nextListingId to assign IDs, but the contract returns the
  // actual Listing structs so we use the tuple data as-is.
  ...
  const veBTCRaw   = (batchResults?.[0]?.result as [RawListing[], bigint] | undefined)?.[0] ?? [];
  const veMEZORaw  = (batchResults?.[1]?.result as [RawListing[], bigint] | undefined)?.[0] ?? [];

  // Deduplicate by (collection, tokenId) in case the contract ever returns dupes
  const allRaw: (RawListing & { collectionKey: "veBTC" | "veMEZO" })[] = [
    ...veBTCRaw.map(l  => ({ ...l, collectionKey: "veBTC"  as const })),
    ...veMEZORaw.map(l => ({ ...l, collectionKey: "veMEZO" as const })),
  ];
  ...
  const listings: Listing[] = allRaw.map((raw, i) => {
    ...
    return {
      listingId:      Number(raw.tokenId), // used as a stable key; not the listing slot ID
      seller:         raw.seller,
